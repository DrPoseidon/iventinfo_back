...
class WorkerController {
  async requisite(req, res) {
    ...
    try {
      const requisite = await sequelize.query(
        "SELECT r.requisite_name, r.requisite_info, r.requisite_status,r.requisite_id FROM requisites r WHERE r.user_id = $user_id",
        {
  ...
  async requisiteadd(req, res) {
    try {
      ...
      await sequelize.query(
        "INSERT INTO requisites (requisite_name, requisite_info, requisite_status, user_id) " +
          "VALUES ($requisite_name, $requisite_info, $requisite_status, $user_id)",
        {
          type: QueryTypes.INSERT,
          bind: {
            requisite_name: requisite_name,
            requisite_info: requisite_info,
  ...
  async purchasecreate(req, res) {
    ...
    await sequelize
      .query(
        "INSERT INTO purchases (purchase_status,requisite_id) " +
          "VALUES ($purchase_status, $requisite_id)",
        {
          type: QueryTypes.INSERT,
          bind: {
  ...
  async upcomingworks(req, res) {
    try {
      ...
      const works = await sequelize.query(
        "SELECT o.order_id, o.event_name, o.event_date, o.event_place, event_status, c.client_name, c.client_phone " +
  ...
  async completeorder(req, res) {
    try {
      ...
      const completed = await sequelize.query(
        "UPDATE orders SET event_status = 'ВЫПОЛНЕНО' WHERE order_id = $order_id",
        {
