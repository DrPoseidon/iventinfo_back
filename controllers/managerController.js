...
class ManagerController {
  async tariffadd(req, res) {
    ...
    res.status(202).send({ message: "Тариф добавлен!" });
  }

  async unprocessedpursh(req, res) {
    const purchases = await sequelize.query(
      "SELECT r.requisite_name, p.purchase_status,p.purchase_id FROM purchases p JOIN requisites r ON (p.requisite_id = r.requisite_id)" +
  ...
  async completepurchase(req, res) {
    try {
      ...
      await sequelize.query(
        "UPDATE purchases SET purchase_status = 'ИСПОЛНЕНО',purchase_price = $purchase_price WHERE purchase_id = $purchase_id",
        {
          type: QueryTypes.UPDATE,
          bind: { purchase_id: id, purchase_price: price },
        }
      );
      res.status(200).send({ message: "Заявка исполнена!" });
    } catch (e) {
      res.send(e);
    }
  }
  ...
  async upcomingworks(req, res) {
    try {
      const works = await sequelize.query(
        "SELECT o.order_id, o.event_name, o.event_date, event_status, c.client_name, c.client_phone,o.event_start_time,o.event_end_time,o.event_place " +
          "FROM user_orders u_o " +
          "JOIN users u ON (u.id = u_o.user_id) " +
          "JOIN orders o ON (o.order_id = u_o.order_id)" +
